/**
 * 仔仔溫書頁面 - 私人版本（需登入）
 * Kids Study Page (Private - requires login)
 * 
 * 課程結構：
 * - 中文：基礎十字主題，分第一課、第二課
 * - 英文：獨立課程
...
function KidsStudyContent() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('kids-study-progress-v2');
    if (saved) {
      // Schedule state update to avoid synchronous setState in effect
  ...
  const saveProgress = (lessonId: string, learnedCount: number) => {
    ...
    localStorage.setItem('kids-study-progress-v2', JSON.stringify(newProgress));
  };
