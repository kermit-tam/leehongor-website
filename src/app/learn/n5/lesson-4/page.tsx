/**
 * 第4課：現在幾點 - 遊戲化版本
 * Lesson 4: What time is it now? - Gamified Version
 */
...
export default function Lesson4Page() {
  ...
  // 初始化本地存儲
  useEffect(() => {
    const savedProgress = localStorage.getItem('lesson4-progress');
    if (savedProgress) {
      setUnitProgress(JSON.parse(savedProgress));
    } else {
      // 初始化進度
      setUnitProgress(lesson4Units.map(u => ({
        unitId: u.id,
        completed: false,
    ...
    const saved = localStorage.getItem('lesson4-sentences');
    if (saved) {
    ...
    const exp = localStorage.getItem('lesson4-exp');
    if (exp) {
  ...
  const saveProgress = useCallback((progress: UnitProgress[]) => {
    ...
    localStorage.setItem('lesson4-progress', JSON.stringify(progress));
  }, []);
  ...
  const saveSentence = useCallback((sentence: SavedSentence) => {
    ...
    localStorage.setItem('lesson4-sentences', JSON.stringify(updated));
    
    // 加EXP
    const newExp = totalExp + 5;
    setTotalExp(newExp);
    localStorage.setItem('lesson4-exp', newExp.toString());
  ...
  const handleQuizComplete = useCallback((result: QuizResult) => {
    ...
    if (user?.uid) {
      ...
      // 更新能力分數（六角雷達）
      if (selectedUnit) {
        updateN5AbilityScores(user.uid, 4, selectedUnit.id, result.score)
  ...
  const progressPercent = (completedCount / lesson4Units.length) * 100;
  // 第4課所有單元都解鎖
  const unlockedUnits = lesson4Units.length;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E5E5E5]">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#4A4A4A] mb-1">第4課：現在幾點</h1>
            <p className="text-[#8C8C8C]">時間表達、動詞、星期、助詞</p>
          </div>
        ...
        <div className="flex justify-between text-sm text-[#8C8C8C] mb-2">
          <span>課程進度</span>
          <span>{completedCount}/{lesson4Units.length} 單元</span>
        </div>
