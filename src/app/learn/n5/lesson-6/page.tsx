...
export default function Lesson6Page() {
  ...
  // 初始化
  useEffect(() => {
    const savedProgress = localStorage.getItem('lesson6-progress');
    if (savedProgress) {
      setUnitProgress(JSON.parse(savedProgress));
    } else {
      setUnitProgress(lesson6Units.map(u => ({
        unitId: u.id,
        completed: false,
    ...
    const saved = localStorage.getItem('lesson6-sentences');
    if (saved) {
    ...
    const exp = localStorage.getItem('lesson6-exp');
    if (exp) {
  ...
  const saveProgress = useCallback((progress: UnitProgress[]) => {
    ...
    localStorage.setItem('lesson6-progress', JSON.stringify(progress));
  }, []);

  const saveSentence = useCallback((sentence: SavedSentence) => {
    const updated = [...savedSentences, sentence];
    setSavedSentences(updated);
    localStorage.setItem('lesson6-sentences', JSON.stringify(updated));
    
    const newExp = totalExp + 5;
    setTotalExp(newExp);
    localStorage.setItem('lesson6-exp', newExp.toString());
  ...
  const handleQuizComplete = useCallback((result: QuizResult) => {
    ...
    if (user?.uid) {
      ...
      // 更新能力分數（六角雷達）
      if (selectedUnit) {
        updateN5AbilityScores(user.uid, 6, selectedUnit.id, result.score)
  ...
  const progressPercent = (completedCount / lesson6Units.length) * 100;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E5E5E5]">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#4A4A4A] mb-1">第6課：食物與喜好</h1>
        ...
        <div className="flex justify-between text-sm text-[#8C8C8C] mb-2">
          <span>課程進度</span>
          <span>{completedCount}/{lesson6Units.length} 單元</span>
        </div>
