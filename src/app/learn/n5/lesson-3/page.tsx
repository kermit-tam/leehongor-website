/**
 * 第3課：這裡是哪裡 - 遊戲化版本
 * Lesson 3: Where is this? - Gamified Version
 */
...
export default function Lesson3Page() {
  ...
  useEffect(() => {
    ...
    const initTimer = setTimeout(() => {
      const savedProgress = localStorage.getItem('lesson3-progress');
      if (savedProgress) {
        setUnitProgress(JSON.parse(savedProgress));
      } else {
        // 初始化進度
        setUnitProgress(lesson3Units.map(u => ({
          unitId: u.id,
          completed: false,
      ...
      const saved = localStorage.getItem('lesson3-sentences');
      if (saved) {
      ...
      const exp = localStorage.getItem('lesson3-exp');
      if (exp) {
  ...
  const saveProgress = useCallback((progress: UnitProgress[]) => {
    ...
    localStorage.setItem('lesson3-progress', JSON.stringify(progress));
  }, []);
  ...
  const saveSentence = useCallback((sentence: SavedSentence) => {
    ...
    localStorage.setItem('lesson3-sentences', JSON.stringify(updated));
    
    // 加EXP
    const newExp = totalExp + 5;
    setTotalExp(newExp);
    localStorage.setItem('lesson3-exp', newExp.toString());
  ...
  const handleQuizComplete = useCallback((result: QuizResult) => {
    ...
    if (user?.uid) {
      ...
      // 更新能力分數（六角雷達）
      if (selectedUnit) {
        updateN5AbilityScores(user.uid, 3, selectedUnit.id, result.score)
  ...
  const progressPercent = (completedCount / lesson3Units.length) * 100;
  // 第3課所有單元都解鎖
  const unlockedUnits = lesson3Units.length;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E5E5E5]">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#4A4A4A] mb-1">第3課：這裡是哪裡</h1>
            <p className="text-[#8C8C8C]">場所指示、建築物、購物會話</p>
          </div>
        ...
        <div className="flex justify-between text-sm text-[#8C8C8C] mb-2">
          <span>課程進度</span>
          <span>{completedCount}/{lesson3Units.length} 單元</span>
        </div>
