...
export default function Lesson6Page() {
  ...
  const saveProgress = useCallback((progress: UnitProgress[]) => {
    ...
    localStorage.setItem('lesson9-progress', JSON.stringify(progress));
  }, []);

  const saveSentence = useCallback((sentence: SavedSentence) => {
    const updated = [...savedSentences, sentence];
    setSavedSentences(updated);
    localStorage.setItem('lesson9-sentences', JSON.stringify(updated));
    
    const newExp = totalExp + 5;
    setTotalExp(newExp);
    localStorage.setItem('lesson9-exp', newExp.toString());
  ...
  const progressPercent = (completedCount / lesson9Units.length) * 100;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E5E5E5]">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#4A4A4A] mb-1">第9課：動詞（過去形）</h1>
        ...
        <div className="flex justify-between text-sm text-[#8C8C8C] mb-2">
          <span>課程進度</span>
          <span>{completedCount}/{lesson9Units.length} 單元</span>
        </div>
    ...
    return (
      <div>
        ...
        <QuizEngine
          ...
          lessonVocab={lesson9Vocab}
          lessonUnits={lesson9Units}
