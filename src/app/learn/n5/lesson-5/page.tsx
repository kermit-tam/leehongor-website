/**
 * 第5課：交通與移動 - 遊戲化試點
 * Lesson 5: Transportation & Movement - Gamified Pilot
 */
...
import { SavedSentence } from './components/sentence-builder';
import { QuizEngine, QuizResult } from './components/quiz-engine';
import { VocabFlashcard } from './components/vocab-flashcard';
import { SentenceBuilder } from './components/sentence-builder';
import { triggerConfetti } from './components/game-feedback';
...
export default function Lesson5Page() {
  ...
  useEffect(() => {
    ...
    const initTimer = setTimeout(() => {
      const savedProgress = localStorage.getItem('lesson5-progress');
      if (savedProgress) {
        setUnitProgress(JSON.parse(savedProgress));
      } else {
        // 初始化進度
        setUnitProgress(lesson5Units.map(u => ({
          unitId: u.id,
          completed: false,
      ...
      const saved = localStorage.getItem('lesson5-sentences');
      if (saved) {
      ...
      const exp = localStorage.getItem('lesson5-exp');
      if (exp) {
  ...
  const saveProgress = useCallback((progress: UnitProgress[]) => {
    ...
    localStorage.setItem('lesson5-progress', JSON.stringify(progress));
  }, []);
  ...
  const saveSentence = useCallback((sentence: SavedSentence) => {
    ...
    localStorage.setItem('lesson5-sentences', JSON.stringify(updated));
    
    // 加EXP
    const newExp = totalExp + 5;
    setTotalExp(newExp);
    localStorage.setItem('lesson5-exp', newExp.toString());
  ...
  const handleQuizComplete = useCallback((result: QuizResult) => {
    ...
    if (user?.uid) {
      ...
      // 更新能力分數（六角雷達）
      if (selectedUnit) {
        updateN5AbilityScores(user.uid, 5, selectedUnit.id, result.score)
  ...
  const progressPercent = (completedCount / lesson5Units.length) * 100;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E5E5E5]">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-2xl font-bold text-[#4A4A4A] mb-1">第5課：交通與移動</h1>
            <p className="text-[#8C8C8C]">交通工具、人物場所、時間表達</p>
          </div>
        ...
        <div className="flex justify-between text-sm text-[#8C8C8C] mb-2">
          <span>課程進度</span>
          <span>{completedCount}/{lesson5Units.length} 單元</span>
        </div>
