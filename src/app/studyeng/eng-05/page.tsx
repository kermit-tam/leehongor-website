      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="flex justify-center gap-2 mb-8">
          {[
            ...
            { id: 'quiz', label: '🎧 聽力', color: 'cyan' },
            { id: 'match', label: '🎮 配對', color: 'sky' },
          ].map((mode) => (
            <button
              key={mode.id}
              onClick={() => {
        ...
        {gameMode === 'learn' && (
          <div className="mt-6 text-center">
            <p className="text-gray-500 text-sm">
              單字 {currentWordIndex + 1} / {lesson5.words.length}
            </p>
            <div className="w-48 h-2 bg-gray-200 rounded-full mx-auto mt-2 overflow-hidden">
              <div
                ...
                style={{ width: `${((currentWordIndex + 1) / lesson5.words.length) * 100}%` }}
              />
            </div>
          </div>
        )}
