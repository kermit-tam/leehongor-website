...
export default function BabySubtraction() {
  ...
  const loadLeaderboard = async () => {
    try {
      const res = await fetch('/api/leaderboard?game=babysubtraction');
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-100 via-rose-100 to-purple-100 flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <h1 className="text-3xl font-black text-pink-600 mb-2">減法BB仔</h1>
          <p className="text-gray-600 mb-2">15題簡單減法</p>
          <p className="text-sm text-gray-500 mb-6">2-10 減 1-9</p>
          
          <div className="space-y-3">
            <button onClick={startGame}
              className="w-full py-4 bg-gradient-to-r from-pink-400 to-rose-500 text-white rounded-xl font-bold text-lg shadow-lg">🎮 開始挑戰</button>
  ...
  if (screen === 'game' && questions.length > 0) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-100 via-rose-100 to-purple-100 flex flex-col items-center justify-center p-4">
        <div className="fixed top-0 left-0 right-0 bg-white/90 backdrop-blur p-4 flex justify-between items-center shadow-lg">
          <div className="text-2xl font-black text-pink-600">👶 {formatTime(timer)}</div>
        ...
        <div className="mt-20">
          <motion.div key={currentQ} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="text-6xl font-black text-pink-600 mb-8">{q.a} − {q.b} = ?</div>
            
            <div className="flex items-center justify-center gap-4 mb-6">
              <input type="number" value={answer} onChange={(e) => setAnswer(e.target.value)} onKeyDown={handleKeyDown}
                className="w-32 h-16 text-4xl font-bold text-center border-4 border-pink-300 rounded-xl focus:border-pink-500 focus:outline-none" autoFocus />
              <button onClick={checkAnswer}
                className="h-16 px-6 bg-gradient-to-r from-pink-400 to-rose-500 text-white rounded-xl font-bold text-xl shadow-lg">確定</button>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-6 max-w-[300px] mx-auto">
            {[1,2,3,4,5,6,7,8,9].map(n => (
              <button key={n} onClick={() => setAnswer(prev => prev + n)}
                className="w-20 h-20 bg-white rounded-2xl text-2xl font-bold text-gray-800 shadow-lg hover:bg-pink-50">{n}</button>
            ))}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-100 via-rose-100 to-purple-100 flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          className="bg-white rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
          <div className="text-6xl mb-4">🎉</div>
          <h1 className="text-3xl font-black text-pink-600 mb-2">挑戰完成！</h1>
          <div className="bg-pink-50 rounded-2xl p-6 mb-6">
            <p className="text-gray-600 mb-2">成績</p>
            <p className="text-5xl font-black text-pink-600">{score} / 15</p>
          ...
          <div className="space-y-3">
            ...
            <button onClick={saveToLeaderboard} disabled={!playerName.trim()}
              className="w-full py-4 bg-gradient-to-r from-pink-400 to-rose-500 text-white rounded-xl font-bold text-lg disabled:opacity-50">🏆 儲存到排行榜</button>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-100 via-rose-100 to-purple-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
        <h1 className="text-3xl font-black text-pink-600 mb-6 text-center">🏆 減法BB仔排行榜</h1>
            ...
            {leaderboard.map((entry, idx) => (
              <div key={idx} className={`flex items-center gap-4 p-4 rounded-xl ${idx === 0 ? 'bg-yellow-100' : idx === 1 ? 'bg-gray-100' : idx === 2 ? 'bg-orange-50' : 'bg-gray-50'}`}>
                ...
                <div className="text-right">
                  <p className="text-xl font-black text-pink-600">{entry.score}分</p>
        ...
        <div className="space-y-3">
          <button onClick={startGame} className="w-full py-4 bg-gradient-to-r from-pink-400 to-rose-500 text-white rounded-xl font-bold text-lg">🎮 再玩一次</button>
