...
export default function LightningSubtraction() {
  ...
  const loadLeaderboard = async () => {
    try {
      const res = await fetch('/api/leaderboard?game=lightningsubtraction');
  ...
  if (screen === 'menu') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-cyan-100 via-blue-100 to-indigo-100 flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          className="bg-white rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
          <div className="text-6xl mb-4">⚡</div>
          <h1 className="text-3xl font-black text-blue-700 mb-2">閃電減法王</h1>
          <p className="text-gray-600 mb-2">15題減法大挑戰</p>
          <p className="text-sm text-gray-500 mb-6">2-18 減 1-9</p>
          
          <div className="space-y-3">
            <button onClick={startGame}
              className="w-full py-4 bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-xl font-bold text-lg shadow-lg">🎮 開始挑戰</button>
            <button onClick={loadLeaderboard}
              className="w-full py-3 bg-gradient-to-r from-yellow-400 to-amber-400 text-white rounded-xl font-bold shadow-lg">🏆 排行榜</button>
            <Link href="/trymath">
  ...
  if (screen === 'game' && questions.length > 0) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-cyan-100 via-blue-100 to-indigo-100 flex flex-col items-center justify-center p-4">
        ...
        <div className="mt-20">
          <motion.div key={currentQ} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="text-6xl font-black text-blue-700 mb-8">{q.a} − {q.b} = ?</div>
            
            <div className="flex items-center justify-center gap-4 mb-6">
              <input type="number" value={answer} onChange={(e) => setAnswer(e.target.value)} onKeyDown={handleKeyDown}
                className="w-32 h-16 text-4xl font-bold text-center border-4 border-blue-300 rounded-xl focus:border-blue-500 focus:outline-none" autoFocus />
              <button onClick={checkAnswer}
                className="h-16 px-6 bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-xl font-bold text-xl shadow-lg">確定</button>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-6 max-w-[300px] mx-auto">
            {[1,2,3,4,5,6,7,8,9].map(n => (
              <button key={n} onClick={() => setAnswer(prev => prev + n)}
                className="w-20 h-20 bg-white rounded-2xl text-2xl font-bold text-gray-800 shadow-lg hover:bg-cyan-50">{n}</button>
            ))}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-cyan-100 via-blue-100 to-indigo-100 flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          className="bg-white rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
          <div className="text-6xl mb-4">🎉</div>
          <h1 className="text-3xl font-black text-blue-700 mb-2">挑戰完成！</h1>
          <div className="bg-blue-50 rounded-2xl p-6 mb-6">
            <p className="text-gray-600 mb-2">成績</p>
            <p className="text-5xl font-black text-blue-600">{score} / 15</p>
          ...
          <div className="space-y-3">
            ...
            <button onClick={saveToLeaderboard} disabled={!playerName.trim()}
              className="w-full py-4 bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-xl font-bold text-lg disabled:opacity-50">🏆 儲存到排行榜</button>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-100 via-blue-100 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
        <h1 className="text-3xl font-black text-blue-700 mb-6 text-center">🏆 閃電減法王排行榜</h1>
        {leaderboard.length === 0 ? <p className="text-gray-500 text-center py-8">暫時未有記錄</p> : (
        ...
        <div className="space-y-3">
          <button onClick={startGame} className="w-full py-4 bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-xl font-bold text-lg">🎮 再玩一次</button>
